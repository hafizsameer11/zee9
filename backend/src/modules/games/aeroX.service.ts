...
async function getGame() {
  ...
  if (!game) throw notFound('AeroX game not configured')
  if (!game.enabled) throw unprocessable('AeroX is currently disabled')
  return game
}
...
async function buildRoundSnapshot(): Promise<RoundSnapshot> {
  ...
  if (!round) throw unprocessable('AeroX unavailable')
