...
async function getGame() {
  ...
  if (!game) throw notFound('Aviator game not configured')
  if (!game.enabled) throw unprocessable('Aviator is currently disabled')
  return game
}
...
async function buildRoundSnapshot(): Promise<RoundSnapshot> {
  ...
  if (!round) throw unprocessable('Aviator unavailable')
