...
async function getGame() {
  ...
  if (!game) throw notFound('Double Crash game not configured')
  if (!game.enabled) throw unprocessable('Double Crash is currently disabled')
  return game
}
...
async function buildRoundSnapshot(): Promise<RoundSnapshot> {
  ...
  if (!round) throw unprocessable('Double Crash unavailable')
