...
export function attachMinesRealtime(_server: HttpServer) {
  const path = `${env.apiPrefix}/games/mines/ws`
  wss = new WebSocketServer({ noServer: true })
  registerGameWs('/games/mines/ws', wss)
  ...
  wss.on('connection', async (ws, req) => {
    try {
      ...
      clients.add(client)
      logger.info({ userId: user.id, clients: clients.size }, 'Mines WS connected')
        ...
        void (async () => {
          try {
            ...
            await dispatchWsAction(ws, msg, {
              start: async (m) => mines.start(client.userId, m.bet, m.mines),
              reveal: async (m) => mines.reveal(client.userId, m.roundId, m.tile),
              cashout: async (m) => mines.cashout(client.userId, m.roundId),
            })
          } catch {
            /* ignore */
          }
  ...
  logger.info({ path }, 'Mines realtime WebSocket attached')
}

export function stopMinesRealtime() {
