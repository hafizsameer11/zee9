...
export function CredsModal({ creds, onClose }: { creds: AgentCreds; onClose: () => void }) {
  ...
  const rows: [string, string][] = [
    ['Panel ID', creds.panelId != null ? String(creds.panelId) : '—'],
    ['Phone (login)', creds.phone],
    ['Password', creds.password],
  ]
  return (
    <Modal title="C2C merchant login" onClose={onClose} foot={<button className="btn btn-primary" onClick={onClose}>Done</button>}>
      <div className="card" style={{ background: 'var(--amber-soft)', boxShadow: 'none', padding: 12, borderRadius: 10, color: '#8a6400', fontSize: 12.5, marginBottom: 16 }}>
        ⚠️ Share these with the merchant for the <b>C2C panel</b> only (not Agent / Mentor panels). Password is shown <b>only once</b>.
      </div>
      {rows.map(([k, v]) => (
