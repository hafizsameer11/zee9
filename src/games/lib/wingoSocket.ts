...
function wsUrlForToken(token: string, mode: string): string {
  const http = new URL(API_BASE)
  const wsProto = http.protocol === 'https:' ? 'wss:' : 'ws:'
  return `${wsProto}//${http.host}${http.pathname.replace(/\/$/, '')}/games/wingo/ws?token=${encodeURIComponent(token)}&mode=${encodeURIComponent(mode)}`
}

export type WingoSocketHandlers = {
...
export function connectWingoSocket(mode: string, handlers: WingoSocketHandlers) {
  ...
  const connect = () => {
    ...
    ws = new WebSocket(wsUrlForToken(token, currentMode))
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg?.type === 'error') handlers.onError?.(msg.message || 'WinGo error')
      } catch {
        /* ignore */
      }
    }
  ...
  return {
    setMode(next: string) {
      ...
      if (ws?.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'mode', mode: next }))
      } else {
        try {
          ws?.close()
        } catch {
