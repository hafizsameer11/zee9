...
import { playZooSfx, preloadZooSfx } from '../services/soundService'
import type { ZooRouletteSfx } from './useZooRoulette'

export function useZooRouletteSound() {
  ...
  const lastAt = useRef<Partial<Record<ZooRouletteSfx, number>>>({})
  ...
  const play = useCallback(
    (id: ZooRouletteSfx, volume = 1) => {
      if (muted) return
      const gap = id === 'trackTick' ? 42 : id === 'chip' || id === 'whoosh' ? 68 : 0
      if (gap) {
        const t = performance.now()
        if (t - (lastAt.current[id] ?? 0) < gap) return
        lastAt.current[id] = t
      }
      playZooSfx(id, volume)
    },
    [muted],
  )
