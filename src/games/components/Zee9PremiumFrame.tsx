...
export default function Zee9PremiumFrame({
  ...
  const nav = [
    { id: 'mines', label: 'Mines', icon: Pickaxe, path: '/play/mines' },
    { id: 'aviator', label: 'Aviator', icon: Plane, path: '/play/aviator' },
    { id: 'teen-patti', label: 'Teen Patti', icon: Spade, path: '/play/teen-patti' },
    { id: 'wingo-lottery', label: 'WINGO', icon: Zap, path: '/play/wingo-lottery' },
    { id: 'double-crash', label: 'DOUBLE CRASH', icon: TrendingUp, path: '/play/double-crash' },
    { id: 'crash', label: 'CRASH', icon: Zap, path: '/play/crash' },
    { id: 'fortune-ox', label: 'FORTUNE OX', icon: Zap, path: '/play/fortune-ox' },
  ]

  const isActive = (id: string) => activeGameId === id || (activeGameId === 'wingo' && id === 'wingo-lottery')
    ...
    <div className={rootClassName} ref={viewportRef}>
      ...
      >
        <div className="bg-[radial-gradient(ellipse_at_top,oklch(0.22_0.04_40),oklch(0.145_0.01_40))] flex flex-col w-full h-full overflow-hidden text-neutral-50">
            ...
            <nav className="flex items-center gap-1 overflow-hidden">
              {nav.map(({ id, label, icon: Icon, path }) => (
                <button
                  ...
                  className={`font-medium rounded-full text-sm flex px-3 py-2 items-center gap-1.5 border-0 cursor-pointer shrink-0 ${
                    isActive(id)
                      ? 'bg-neutral-800 text-neutral-50 border border-white/15'
                      : 'bg-transparent text-[#a1a1a1] hover:text-neutral-200'
                  }`}
                >
          ...
          <div className="min-h-0 flex-1 px-8 pb-4 overflow-hidden">{children}</div>
